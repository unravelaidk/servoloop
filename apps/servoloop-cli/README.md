...
live `run --prompt TEXT` executions; diagnostics always go to standard error.
...
syncs append-only journal records before dispatch. It is journal-only in this
slice; session snapshots and atomic config writes are not implemented. The
store rejects symlink journal paths, but like ordinary filesystem checks it
cannot eliminate every time-of-check/time-of-use race with another process.
